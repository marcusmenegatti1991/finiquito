...
export default function CalculadoraFiniquito() {
  ...
  const [diasVacacionesManual, setDiasVacacionesManual] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (params.get("v")) {
      setDiasVacaciones(parseInt(params.get("v")!));
      setDiasVacacionesManual(true);
    }
  ...
  useEffect(() => {
    if (diasVacacionesManual) return;
    try {
      const ingreso = new Date(fechaIngreso);
      const terminacion = new Date(fechaTerminacion);
      if (isNaN(ingreso.getTime()) || isNaN(terminacion.getTime())) return;
      const diasLFT = calcularDiasVacacionesLFT(ingreso, terminacion);
      if (diasLFT > 0) setDiasVacaciones(diasLFT);
    } catch {}
  }, [fechaIngreso, fechaTerminacion, diasVacacionesManual]);

  const diasVacacionesLFT = useMemo(() => {
    try {
      return calcularDiasVacacionesLFT(new Date(fechaIngreso), new Date(fechaTerminacion));
    } catch {
      return 0;
    }
  }, [fechaIngreso, fechaTerminacion]);
  ...
  const handleSalarioBlur = () => {
    setSalarioInput(new Intl.NumberFormat("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(salarioMensual));
  };
...
<body>
  <div class="header">
    <h1>Calculadora de Finiquito Mexico</h1>
    <p>Calculo basado en la Ley Federal del Trabajo - finiquito.xyz</p>
  </div>
  <div class="total-box">
  ...
  <div class="section-title">Informacion Laboral</div>
  <div class="info-grid">
    ...
    <div class="info-item"><div class="key">Fecha de Terminacion</div><div class="val">${formatearFecha(new Date(fechaTerminacion))}</div></div>
    <div class="info-item"><div class="key">Dias No Pagados</div><div class="val">${diasNoPagados} dias</div></div>
    <div class="info-item"><div class="key">Dias de Vacaciones</div><div class="val">${diasVacaciones} dias</div></div>
  </div>
  ...
  <div class="footer">
    Este documento es una estimacion basada en la Ley Federal del Trabajo de Mexico. No constituye asesoria legal.<br>
    Generado en finiquito.xyz
  </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="salario" className="font-semibold">Salario Mensual</Label>
                  <Tooltip>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="ingreso" className="font-semibold">Fecha de Ingreso</Label>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Info className="h-4 w-4 text-purple-600 cursor-help" />
                    </TooltipTrigger>
                ...
                <Input id="ingreso" type="date" value={fechaIngreso} onChange={(e) => setFechaIngreso(e.target.value)} />
              </div>

              {/* Fecha Terminación */}
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="terminacion" className="font-semibold">Fecha de Terminación</Label>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Info className="h-4 w-4 text-purple-600 cursor-help" />
                    </TooltipTrigger>
                ...
                <Input id="terminacion" type="date" value={fechaTerminacion} onChange={(e) => setFechaTerminacion(e.target.value)} />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Label htmlFor="vacaciones" className="font-semibold">Días de Vacaciones Correspondientes</Label>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Info className="h-4 w-4 text-purple-600 cursor-help" />
                      </TooltipTrigger>
                      <TooltipContent className="bg-purple-50 text-purple-900 border-purple-200">
                        Calculado según Vacaciones Dignas 2023 (Art. 76 LFT). Puedes ajustarlo si tu contrato da más días.
                      </TooltipContent>
                    </Tooltip>
                  </div>
                  {diasVacacionesManual && diasVacacionesLFT > 0 && (
                    <button
                      type="button"
                      onClick={() => { setDiasVacacionesManual(false); setDiasVacaciones(diasVacacionesLFT); }}
                      className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 underline"
                    >
                      <RotateCcw className="h-3 w-3" />
                      Usar LFT ({diasVacacionesLFT} días)
                    </button>
                  )}
                </div>
                <Input
                  ...
                  onChange={(e) => { setDiasVacaciones(parseFloat(e.target.value) || 0); setDiasVacacionesManual(true); }}
                />
                {!diasVacacionesManual && diasVacacionesLFT > 0 && (
                  <p className="text-xs text-green-700">✅ Calculado automáticamente según LFT (Vacaciones Dignas 2023)</p>
                )}
                {diasVacacionesManual && diasVacacionesLFT > 0 && diasVacaciones < diasVacacionesLFT && (
                  <p className="text-xs text-amber-700">⚠️ La LFT indica {diasVacacionesLFT} días para tu antigüedad. Verifica con tu empresa.</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="nopagados" className="font-semibold">Días Laborados No Pagados</Label>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Info className="h-4 w-4 text-purple-600 cursor-help" />
                    </TooltipTrigger>
              ...
              <div className="space-y-3 pt-4 border-t">
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="radio" checked={!esLiquidacion} onChange={() => setEsLiquidacion(false)} className="w-4 h-4" />
                    <span className="text-sm">Finiquito (renuncia o despido justificado)</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="radio" checked={esLiquidacion} onChange={() => setEsLiquidacion(true)} className="w-4 h-4" />
              ...
              <Card className="border-4 border-primary bg-gradient-to-br from-blue-50 to-white">
                <CardContent className="pt-6">
                  <div className="text-center">
                    ...
                    <p className="text-5xl font-bold text-primary mb-4">{formatearMoneda(resultado.totalFiniquito)}</p>
                    <div className="flex gap-2 justify-center flex-wrap">
                      <Button onClick={descargarPDF} variant="outline" size="sm" className="gap-2">
                        <Download className="h-4 w-4" />
                        Descargar PDF
                      </Button>
                      <Button onClick={compartirResultados} variant="outline" size="sm" className="gap-2">
                      ...
                      <CardContent className="pt-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            ...
                            {concepto.formula && (
                              <p className="text-xs text-purple-600 font-mono bg-purple-50 p-2 rounded">{concepto.formula}</p>
                            )}
                          </div>
                          <div className="text-right">
                            <p className="text-2xl font-bold text-primary">{formatearMoneda(concepto.monto)}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  <Card>
                    <CardContent className="pt-4 space-y-3">
                      ...
                      <div className="flex justify-between py-2 border-b">
                        ...
                        <span className="font-semibold">{formatearFecha(new Date(fechaIngreso))} a {formatearFecha(new Date(fechaTerminacion))}</span>
                      </div>
                    </CardContent>
                  </Card>
                </TabsContent>
